...
public class PrefsCursor extends AbstractCursor {
     ...
     */
    public PrefsCursor(String[] columnNames, int initialCapacity) {
     ...
     *                    determines column ordering elsewhere in this cursor
     */
    public PrefsCursor(String[] columnNames) {
        this(columnNames, 16);
    }
    ...
    public int getType(int column) {
        return Util.getTypeOfObject(get(column));
    }
    ...
    public static class Util {
        public static boolean getBoolean(int obj) {
            return obj == 1;
        }

        public static int booleanToInt(boolean vb) {
            int i = (vb ? 1 : 0);
            return i;
        }

        public static byte[] setToBlob(Set<String> vset) {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            BufferedWriter bw = null;
            try {
                bw = new BufferedWriter(new OutputStreamWriter(bos));
                Iterator<String> it = vset.iterator();
                while (it.hasNext()) {
                    bw.write(it.next());
                    bw.write("\n");
                }
                ...
                cm.android.preference.util.Util.getLogger().error(e.getMessage(), e);
            } finally {
                cm.android.preference.util.Util.closeQuietly(bw);
            }

            return bos.toByteArray();
        }

        public static Set<String> getSet(byte[] bs) {
            BufferedReader br = null;
            HashSet<String> set = new HashSet<>(bs.length);
            try {
                String str = null;
                ...
                cm.android.preference.util.Util.closeQuietly(br);
            }
